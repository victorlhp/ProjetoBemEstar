...
import { saveUserData } from './firestoreService'; // Importa a função de salvar dados
  ...
  useEffect(() => {
    const calcularPontuacao = async () => {
      try {
        ...
        // Obtém as informações do usuário do AsyncStorage ou outra fonte
        const userEmail = await AsyncStorage.getItem('userEmail') || '';
        const userName = await AsyncStorage.getItem('userName') || '';
        setEmail(userEmail);
        setNome(userName);

        // Chama a função para salvar os dados no Firestore
        await saveUserData(email, nome, respostas, {
          ansiedade: pontuacaoAns,
          depressao: pontuacaoDep,
        });
